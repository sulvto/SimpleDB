...
public class BufferPool {
    ...
    private Map<PageId, Integer> useCount;
    ...
    public BufferPool(int numPages) {
        ...
        this.pool = new HashMap<>(numPages);
    ...
    public Page getPage(TransactionId tid, PageId pid, Permissions perm)
        ...
        if (pool.containsKey(pid)) {
            this.useCount.put(pid, (this.useCount.getOrDefault(pid, 0)) + 1);
            return pool.get(pid);
        } else {
    ...
    */
    ...
    private synchronized void evictPage() throws DbException {
        ...
        Optional<PageId> pageIdOptional = pool.keySet()
                .stream()
                .sorted(Comparator.comparingInt(o -> this.useCount.getOrDefault(o, 0)))
                .findFirst();
        if (pageIdOptional.isPresent()) {
            PageId pageId = pageIdOptional.get();
            if (pool.get(pageId).isDirty() != null) {
                try {
                    flushPage(pageId);
                    discardPage(pageId);
                } catch (IOException e) {
